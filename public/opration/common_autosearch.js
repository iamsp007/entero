...
window.initialize = function () {
    ...
    const fieldsConfig = [
        { id: "#address1", zip: "#zipcode", city: "#city", state: "#state", instanceKey: "autocomplete1" },
        { id: "#secondAddress1", zip: "#secondZipcode", city: "#secondCity", state: "#secondState", instanceKey: "autocomplete2" },
        { id: "#thirdAddress1", zip: "#thirdZipcode", city: "#thirdCity", state: "#thirdState", instanceKey: "autocomplete3" },
        { id: "#forthAddress1", zip: "#forthZipcode", city: "#forthCity", state: "#forthState", instanceKey: "autocomplete4" },
        { id: "#fiveAddress1", zip: "#fiveZipcode", city: "#fiveCity", state: "#fiveState", instanceKey: "autocomplete5" },
        { id: "#caregiverAddress1", zip: "#caregiverZipcode", city: "#caregiverCity", state: "#caregiverState", instanceKey: "autocomplete6" }, 
        { id: "#address1_form", zip: "#zipcode_form", city: "#city_form", state: "#state_form", instanceKey: "autocomplete7" },       
    ];
 
    // Initialize the address fields for autocomplete
    fieldsConfig.forEach(config => {
        initAddressAutocomplete(config.id, config.zip, config.city, config.state, config.instanceKey);
    });
};
 
// Initialize autocomplete for a specific address field
function initAddressAutocomplete(addressSelector, zipSelector, citySelector, stateSelector, instanceKey) {
    ...
    addressField.addEventListener("keyup", function () {
        if (addressField.value.length >= 3) {
            if (!autocompleteInstances[instanceKey]) {
                setupAutocomplete(addressField, zipField, cityField, stateField, instanceKey);
            }
        }
    });
}
...
function setupAutocomplete(addressField, zipField, cityField, stateField, instanceKey) {
